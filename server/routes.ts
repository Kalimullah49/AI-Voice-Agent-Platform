...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get Vapi token for client-side web SDK
  app.get("/api/vapi/web-token", (req, res) => {
    try {
      if (!process.env.VAPI_AI_TOKEN) {
        return res.status(400).json({
          success: false,
          message: "Vapi.ai API token is not configured"
        });
      }
      
      // Return token for client-side use with Web SDK
      res.json({
        success: true,
      ...
      res.status(500).json({
        success: false,
        message: "Error fetching Vapi token"
      });
    }
  });
